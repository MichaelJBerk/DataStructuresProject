...
public class HashTableImpl<Key, Value> implements HashTable<Key, Value> {
    ...
    private class ValList {
        ...
        private Value get(Key k) {
            ListElement cElement = this.head;
            if (cElement.key.equals(null)) {
                return null;
            }
            while (!cElement.key.equals(k)) {
    ...
    private void doubleArray() {
        int doubleSize = this.table.length * 2;
        HashTableRow[] tableCopy = this.table.clone();
        HashTableRow<?, ?>[] newTable = new HashTableRow[doubleSize];
        this.table = newTable;
        for (int i = 0; i < tableCopy.length; i++) {
            ValList list = tableCopy[i].value;
            ListElement cElement = list.head;
            this.put(cElement.key, cElement.value);
            while (cElement.next != null) {
                cElement = cElement.next;
                this.put(cElement.key, cElement.value);
            }
        }
    ...
    private int hashFunction(int tableSize, Object key) {
        return (key.hashCode() & 0x7fffffff) % tableSize;
    }

    private int hashFunction(Object key) {
        return hashFunction(this.table.length, key);
    }
    ...
    /**
     * @return the space of the table that's currently used
     */
    private int lengthUsed() {
        int usedSpace = 0;
        for (HashTableRow hr : this.table) {
            if (hr != null) {
                usedSpace++;
            }
        }
    ...
    public Value put(Key k, Value v) {
        ...
        if (this.lengthUsed() == this.table.length) {
            doubleArray();
        }
        int i = this.hashFunction(k);
        Value old = null;
